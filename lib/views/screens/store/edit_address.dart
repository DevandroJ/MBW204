...
class _EditAlamatPageState extends State<EditAlamatPage> {
  ...
  void initState() {
    super.initState();
    detailAddressController = TextEditingController(text: widget.address);
    typeAddressController = TextEditingController(text: widget.typeAddress);
    subDistrictController = TextEditingController(text: widget.subDistrict);
    phoneController = TextEditingController(text: widget.phoneNumber);
    postCodeController = TextEditingController(text: widget.postalCode);
  ...
  Future submit() async {
    try {
      ...
      if(typeAddressController.text.trim().isEmpty) {
        Fluttertoast.showToast(
          backgroundColor: ColorResources.ERROR,
          msg: "Lokasi Alamat tidak boleh kosong",
          fontSize: 14.0,
        );
        return;
      }
      ...
      if(subDistrictController.text.trim().isEmpty) {
        Fluttertoast.showToast(
          backgroundColor: ColorResources.ERROR,
      ...
      await Provider.of<RegionProvider>(context, listen: false).postEditDataAddress(
        context, 
        widget.idAddress,
